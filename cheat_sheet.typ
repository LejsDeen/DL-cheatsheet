...
where $X^* = lim_(delta -> 0) (X^top X + delta I)^(-1) X^top$ Moore-Penrose inv.
...
Polynomials, ridge functions (functions of the form $phi(a^top x + b)$) and MLPs with $C^oo$ activations are universal approximators. \
...
*Gradient flow ODE* $(dif x) / (dif t) = -nabla f(x)$ gives ideal trajectory to be approximated by GD.
...
*Strict convexity*: Replace $<=$ in convexity with $<$,
for twice differentiable $f$: $nabla^2 f(x) ≻ 0$ $forall x$; strict convexity is *weaker* than strong convexity ($<-$)!
...
For 2D convolution $y = x * w$, $(diff L)/(diff w) = x * (diff L)/(diff y)$
]

== Convolutional Networks

*Conventions* for Padding: Add $0$ around input.

*ConvNets for Images* ($r$ out channel, $u$ in ch)\
...
*Group* is set $G$ with a binary operation s.t.: 1) $(g h) f = g (h f)$, 2) $exists e in G$ s.t. $e f = f e = f$, 3) $forall g space exists g^(-1) in G$ s.t. $g g^(-1) = g^(-1) g = e$, 4) $g h in G space forall g, h$. $quad$ *Abelian* if $g h = h g$.

== Sets and Points

*Order-invariance property:* \
$f(x_1, ..., x_M) = f(x_(pi(1)), ..., x_(pi(M)))$ (perturbs).

*(Permutation) Equivariance property:* \
$f(x_1, ..., x_M) = (y_1, ..., y_M) => f(x_(pi(1)), ..., x_(pi(M))) = (y_(pi(1)), ..., y_(pi(M)))$

*Attn wo/ mask is permutation-equivariant*
...
$P in {0, 1}^(M times M)$ w/ single 1 in each row and col.
...
#text(size:0.93em)[$X_m = {{x_n : {v_n, v_m} in E}}$, ${{dot.c}} = "multiset"$]

*Message passing scheme* \
$phi(x_m, X_m) = phi(x_m, plus.o.big_(x in X_m) psi(x))$, \
$plus.o$ is some permutation-invariant operation.

#colorbox(title:"Normalized adjacency matrix")[

$overline(A) = D^(-1/2) (A + I) D^(-1/2)$, 
$D = "diag"(d_1, ..., d_M)$,
#text(size:0.9em)[$d_m = 1 + sum_(n=1)^M a_(n m)$.
$lambda_"max" (overline(A)) = 1$, $lambda_"min" (overline(A)) >= -1$. ]
\ For $overline(L)=I-overline(A)$,
$lambda_"min" (overline(L))=0$ and $lambda_"max" (overline(L)) <= 2$.
]
If $tilde(A)$ symmetric, $tilde(A) = sum_i lambda v_i v_i^top$.

For $overline(A)$, $v_1 prop [ - sqrt(d_i) -]^top$, where d is degree+1.
...
#text(size:0.93em)[$x_i^(l+1) = sum_j p_(i j)(L) x_j^l + b_i$;
...
*Implicit bias towards min norm solutions*: Any convergent algorithm with iterates in $"span"{x_1, ..., x_n}$ finds minimum norm solution.
...
- #text(size:0.95em)[$op("KL")(Q||P)$: "information cost" of moving $P -> Q$] \
...
With residuals $tilde(y)_i = y_i - f(bold(theta)_0)(x_i)$, training becomes *linear regression* with features $nabla f(bold(theta)_0)(x_i)$: $min_beta sum_i norm(tilde(y)_i - beta^top  nabla f(theta_0)(x_i))^2$
...
Init: $w_(i j)^((ell)) tilde (sigma_w)/sqrt(m_ell) cal(N)(0,1)$.
...
Kernel  $nabla K = nabla^2 f(x) nabla f(z) + nabla^2 f(z) nabla f(x)$,
...
*Marginals + Conditionals of MV Gaussians* \
...
Same result as ridge: \
$hat(bold(w)) = (bold(X)^top bold(X) + sigma^2 bold(I)_d)^(-1) bold(X)^top bold(y)$.
...
- Any $bold(C) = bold(A)^(-1) (bold(U)_k^*)^top$, $bold(D) = bold(U)_k^(*) bold(A)$ is optimal
...
#colorbox[
$p(x;bold(mu), bold(Sigma)) = exp[-1/2 (x - bold(mu))^top bold(Sigma)^(-1) (x - bold(mu))] / sqrt((2 pi)^n "det"(bold(Sigma)))$]
...
$bold(w)_i = rho_i bold(u)_i, quad rho_i^2 = max{0, lambda_i - sigma^2}$. $W = U_m L_m$, where $(lambda_i, bold(u)_i)$ is $i$-th eigenpair of data covariance.
...
*MC estimation*: $E_(q_phi.alt (bold(z)|bold(x)))[log p_theta (bold(x)|bold(z))] approx -1/(2sigma^2 K) sum_(k=1)^K norm(bold(x) - bold(mu)_theta (bold(z_k)))^2_2 - d/2 log(2 pi sigma^2)$.
...
*Diffeomorphism:* $T$ is bijective, differentiable, w/ differentiable inverse. Guarantees $det J_T eq.not 0$.
...
*GAN Objective:* \
#text(size:0.78em)[
$min_theta max_phi underbrace(EE_(x tilde.op p_r)[log D_phi (x)], "real samples") + underbrace(EE_(z tilde.op p_z)[log(1 - D_phi (G_theta (z)))], "fake samples")$]
...
#colorbox(color:purple)[Common alternative objective for the generator is to maximize $EE_(z ~p(z))[log D(G(z))]]$ instead of minimizing $EE_(z ~ p(z)) [log(1-D(G(z)))]$ to help mitigate vanishing gradient problem when discriminator becomes to good, i.e. $D(G(z)) -> 0$. \
...
Binary clfcation with $p(y=1) = p(y=0) = 1/2$:
...
*Generator Logistic Objective = JS Div:* \
#text(size:0.9em)[
...
Training is *Saddle-point problem*, unstable!
...
*Fixed Fwd process:* Gradually add Gaussian noise over $T$ steps until data becomes pure noise.

Forward step: $q(x_t | x_(t-1)) = cal(N)(x_t; sqrt(1 - beta_t) x_(t-1), beta_t I)$.
...
For small $beta_t$, reverse $q(x_(t-1)|x_t)$ is also Gaussian.
...
5. GD step on $nabla_theta ||epsilon - epsilon_theta (x_t, t)||^2$
  ]],
  [#text(0.75em)[
...
*Backpropagation for example network*
- $a_t = F(x_t, h^((t-1)), y^((t-1)); theta)$
- $h_t = sigma(a_t)$
- $hat(y_t) = G(h_t; phi)$
- $L_t = H(hat(y_t), y_t)$
- $L = sum_(t=1)^T L_t$
